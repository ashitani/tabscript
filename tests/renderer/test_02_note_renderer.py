...
def test_render_note_chord(note_renderer, pdf_canvas):
    """和音の音符の描画テスト"""
...
def test_draw_chord_notes(note_renderer, pdf_canvas):
    """和音の音符の描画テスト"""
    note = Note(string=1, fret="5", duration="4", is_chord=True, is_chord_start=True)
    note.chord_notes = [
        Note(string=2, fret="5", duration="4"),
    ...
    note_renderer._draw_chord_notes(pdf_canvas, 100, 100, note)
    # 実際の描画結果は確認できないため、エラーが発生しないことを確認 